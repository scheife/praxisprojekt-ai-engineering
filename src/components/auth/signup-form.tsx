...
export function SignupForm() {
  const [state, formAction, isPending] = useActionState(signup, EMPTY)
  ...
  return (
    <form action={formAction} noValidate className="flex flex-col gap-5">
      ...
      {/* Gesperrt, solange abgeschickt wird: so entsteht bei einem Doppelklick genau ein
          Konto und niemand sieht einen Fehler (EC-1). */}
      <Button type="submit" disabled={isPending} className="h-9 w-full font-grotesk">
        {isPending ? 'Moment …' : 'Konto anlegen'}
      </Button>
    </form>
  )
}
