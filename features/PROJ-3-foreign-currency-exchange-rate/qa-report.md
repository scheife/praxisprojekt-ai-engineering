...
- [x] **AC-6** — die Währung bleibt nach dem Erfassen stehen · *Evidenz: der Rücksetz-Effekt in `expense-composer.tsx` fasst `setCurrency` **null**-mal an, während `setAmount('')` und `setNote('')` laufen. **Im Browser nicht nachgestellt** — siehe Not Verified*
...
**19 von 19 Acceptance Criteria erfüllt** — zwei davon (AC-6, AC-11) am Code statt an der laufenden Oberfläche, siehe *Not Verified*.
...
- [ ] **EC-9** — **BUG-1.** Bei nicht erreichbarer Datenbank erscheint **keine** verständliche Meldung: die Anfrage hängt **50,4 Sekunden** und endet mit **HTTP 500** und leerem Rumpf. Ein Kursproblem und ein Datenbankproblem sind damit zwar nicht zu verwechseln — aber EC-9 verlangt ausdrücklich, dass PROJ-2s EC-4 weiter gilt, und das tut es nicht. **Nicht von PROJ-3 verursacht**, siehe BUG-1
...
**18 von 18 grün** (Chromium und Mobile Safari, 1,8 min) — als Regression gegen PROJ-1 und PROJ-2.
Für PROJ-3 selbst existiert **noch keine** E2E-Journey; das ist die wichtigste offene Lücke, siehe
*Not Verified*.
...
- [!] **AC-6 und AC-11 sind am Code belegt, nicht an der laufenden Oberfläche** — dass die Währung
...
### BUG-1: Bei nicht erreichbarer Datenbank hängt die Erfassung 50 Sekunden und endet mit HTTP 500
...
- **Acceptance Criteria:** **19 von 19 erfüllt** (AC-6 und AC-11 am Code, nicht an der Oberfläche)
- **Edge Cases:** **8 von 9 erfüllt** — EC-9 offen als BUG-1
- **Bugs:** 0 Critical · 0 High · **1 Medium** (BUG-1, vorbestehend, betrifft PROJ-2 gleichermaßen) · 0 Low
...
- **Production Ready:** **JA** — kein Critical- und kein High-Befund, und alle 19 AC wurden in diesem
  Lauf ausgeführt, die meisten gegen die laufende Anwendung und den **echten** Kursdienst.

**Das „JA" ist eine Aussage über gefundene Fehler, nicht über vollständige Abdeckung.**

- **Ein Medium-Befund steht offen** und gehört nach PROJ-2, nicht hierher.
- **Die Liste unter *Not Verified* ist nicht leer**, und ein Punkt darin wiegt schwerer als die
  anderen: **Der Weg „Fremdwährung im Browser auswählen und erfassen" ist nicht belegt.** Alles
  spricht dafür, dass er trägt — der Servercode ist geprüft, der Euro-Weg läuft im Browser, die
  Auswahl folgt exakt dem Muster der Kategorie aus PROJ-2 —, aber geprüft ist er nicht. Solange das
  so bleibt, ist `/e2e-tests` keine Kür, sondern der nächste Schritt.
