...
| TD-27 | Der Abmelde-Knopf verlässt die Konto-Karte; die **eine** Schaltfläche steht im `AppHeader` von PROJ-2 (AC-19) | Zwei gleich benannte Schaltflächen auf einer Seite sind für Screenreader nicht unterscheidbar und haben die E2E-Suite mit `strict mode violation` zerbrochen. Die Action selbst bleibt unverändert — der Header ruft schon heute **dieselbe** `logout()`-Action auf, es entsteht kein zweiter Weg | Beide behalten und unterschiedlich benennen („Von diesem Gerät abmelden"); oder den Knopf aus dem Header nehmen | Die Konto-Karte trägt danach nur noch die E-Mail-Adresse. Das ist wenig, aber es ist die Frage, die eine Kontoseite beantworten soll: mit welchem Konto bin ich hier | 2026-08-31 |
...
- [x] ~~**AC-18 verlangt „unter 500 ms je Antwort", unter Last gibt es Ausreißer darüber.**~~
      **Beantwortet am 29.08.2026 durch `/refine`.** AC-18 misst die Zeitzusage jetzt als Perzentil
      am eingeschwungenen Produktions-Build: mindestens 95 % unter 500 ms, keine Antwort über
      1 Sekunde, und die erste Anfrage nach einem Serverstart zählt nicht mit. Ausschlaggebend war
      die Messung aus Lauf 7: erste Anfrage 717 ms, danach sofort 361 ms — ein Maximum als Kriterium
      hätte damit das Hosting gemessen, nicht das Feature. Die Grenze ganz zu streichen kam nicht in
      Frage, weil sie die billige Lösung verhindert (alles gleichmäßig langsam machen erfüllt die
      Ununterscheidbarkeit trivial); deshalb Perzentil **plus** harter Deckel. **Am Code ändert sich
      nichts** — die Untergrenze aus TD-20 bleibt bei 350 ms, und der gemessene Zustand erfüllt die
      neue Fassung bereits (48 von 48 unter 500 ms, Maximum 464 ms).
